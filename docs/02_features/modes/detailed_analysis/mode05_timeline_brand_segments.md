# モード5 - ファネル分析②（ブランド×複数セグメント）要件定義書
...
**モード4**（`timeline_segment_brands`）: ファネル分析②（セグメント固定×複数ブランド）
...
**手順**:
1. モード5「ファネル分析②（ブランド×複数セグメント）」を選択
2. ブランド「自社ブランド」を選択
...
### 10.1 モード4との完全な対称性
モード4（セグメント固定）とモード5（ブランド固定）は、固定軸と比較軸が異なるだけで構造は完全に同じ：
...
1. モードID: `timeline_segment_brands` → `timeline_brand_segments`
...
モード5「ファネル分析②（ブランド×複数セグメント）」は、特定のブランドを複数のセグメント間で時系列指標により比較する分析モードです。
